...
class ContinuityTransitionAnimator: NSObject, UIViewControllerAnimatedTransitioning {
    ...
    func animateTransition(using transitionContext: UIViewControllerContextTransitioning) {
        ...
        source.prepareForTransition(to: destination, withDuration: transitionDuration(using: transitionContext)) { finished in
            ...
            /*At this point, the source has been able to prepare. The implicit requirement for this particular transition is that
             the view needs to be clear at this point. Now, assuming the backgrounds of the views match we can swap in the
             destination (with no visual effect - so it all looks like one controller) and allow it to animate it's elements on screen. */
            ...
            /* Now, the destination is on screen and ready to complete the transition. But we don't need to include that 'entrance'
             animation in our transition. So we'll simultaneously give our source time to clean up after transitioning to the
             destination, tell the destination to perform any 'complete animations/work' and tell the context we're finished. */
